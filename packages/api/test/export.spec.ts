...
describe("GET /api/export/:bucket", () => {
	it("returns 401 when unauthenticated", async () => {
		const res = await SELF.fetch(
			"https://example.com/api/export/foundations"
		);
	...
	it("returns 404 for invalid bucket", async () => {
		const res = await SELF.fetch(
			"https://example.com/api/export/invalid-bucket",
			{
	...
	it("returns correct content-type and content-disposition headers", async () => {
		...
		expect(res.headers.get("content-disposition")).toBe(
			'attachment; filename="foundations.md"'
		);
	});
	...
	it("filters by bucket correctly", async () => {
		...
		const res2 = await SELF.fetch(
			"https://example.com/api/export/backend",
			{
				headers: { cookie: authCookie },
			}
		);
	...
	it("works with all valid bucket slugs and titles", async () => {
		...
		for (const bucket of buckets) {
			const res = await SELF.fetch(
				`https://example.com/api/export/${bucket.slug}`,
				{
					headers: { cookie: authCookie },
				}
			);
			...
			expect(res.headers.get("content-disposition")).toBe(
				`attachment; filename="${bucket.slug}.md"`
			);
		}
	});

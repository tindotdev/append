      ...
      setBatch((prev) => {
        ...
        return {
          ...
          candidates: prev.candidates.map((c) =>
            c.id === cand.id ? response.candidate : c
          ),
        };
      });
    ...
    <div className="max-w-4xl">
      ...
      <div className="mt-6 border border-zinc-800 rounded-lg divide-y divide-zinc-800">
        {batch.candidates.map((cand, index) => {
          ...
          const isSuggestionPending = cand.suggestedBucket === null && cand.suggestionStatus !== "in_progress" && cand.suggestionStatus !== "error";
            ...
            <div key={cand.id} className="p-4">
              ...
              {(cand.suggestedBucket || cand.suggestedText) && (
                <div className="ml-10 mb-3 p-2 bg-zinc-900 rounded text-sm text-zinc-400">
                  ...
                  {cand.suggestedBucket && (
                    <span className="text-zinc-300">{cand.suggestedBucket}</span>
                  )}
                  {cand.suggestedBucket && cand.suggestedText && (
                    <span className="text-zinc-500"> — </span>
                  )}
                  {cand.suggestedText && (
                    <span className="text-zinc-300">{cand.suggestedText}</span>
                  )}
                </div>
              )}
              ...
              <div className="ml-10 flex flex-col gap-3">
                ...
                {/* Row error */}
                {rowError && (
                  <p className="text-red-400 text-sm">{rowError}</p>
                )}
              </div>
            </div>
          );
